...
select count(1) into v_count from cportal_prod.ws_termmeta  where term_id=p_termid and meta_key=v_fname collate utf8mb4_unicode_ci;
if(v_count=0) then
begin
	select max(meta_id) + 1 into v_new_meta_id  from cportal_prod.ws_termmeta;

	insert into cportal_prod.ws_termmeta (meta_id,term_id,meta_key,meta_value) values(v_new_meta_id,p_termid,v_fname,v_fvalue); 
end;
else
begin
	update cportal_prod.ws_termmeta set meta_value=v_fvalue where term_id=p_termid and meta_key=v_fname collate utf8mb4_unicode_ci; 
end;
end if;
if done then 
	leave cur; 
...
select option_id into v_optionid from cportal_prod.ws_options where option_value like concat('%','s:15:"attribute_label";s:',length(p_attr),':"',p_attr,'";%') collate utf8mb4_unicode_ci and   option_name='_transient_wc_attribute_taxonomies'; 

if v_optionid is null then
begin
	
    select option_value into v_options from cportal_prod.ws_options where option_name='_transient_wc_attribute_taxonomies'; 
	...
	update cportal_prod.ws_options 
	...
	from cportal_prod.ws_term_taxonomy tx 
	inner join cportal_prod.ws_terms ter on ter.term_id=tx.term_id
	inner join cportal_prod.ws_icl_translations icl on  icl.element_id=ter.term_id
...
begin
	...
	select max(term_id)+1 into v_term_id from cportal_prod.ws_terms;
	insert into cportal_prod.ws_terms(`term_id`,`name`,`slug`) values(v_term_id,p_value,concat(replace(p_value,' ','-'),'_',p_lang));
	
    select max(term_taxonomy_id)+1 into v_term_taxonomy_id from cportal_prod.ws_term_taxonomy; 
	insert INTO cportal_prod.ws_term_taxonomy(`term_taxonomy_id`,`term_id`,`taxonomy`,`description`,`parent`,`count`) values( v_term_taxonomy_id,v_term_id,p_type,'',0,0) ;          
    ...
    begin
        ...
        if (v_trid>0) then
        begin
			insert into cportal_prod.ws_icl_translations (element_type, element_id,trid, language_code,source_language_code)
    ...
    begin
        ...
        select max(trid)+1 into v_trid from cportal_prod.ws_icl_translations;
		insert into cportal_prod.ws_icl_translations (element_type, element_id,trid, language_code,source_language_code)
...
begin
     ...
     INSERT INTO cportal_prod.`wp_woocommerce_attribute_taxonomies` (`attribute_label`, `attribute_name`, `attribute_type`, `attribute_orderby`, `attribute_public`) VALUES (p_attr,replace( p_attr,' ','-'), 'select', 'menu_order', 0);
...
begin
	...
	select max(term_id)+1 into v_term_id from cportal_prod.ws_terms;
	insert into cportal_prod.ws_terms(`term_id`,`name`,`slug`) values(v_term_id,p_value,lower(concat(replace(p_value,' ','-'),'_',p_lang))); 
...
select count(1) into v_temp_counter from cportal_prod.ws_postmeta  where post_id=v_postid and meta_key='hide_product';
if v_temp_counter>0 then

	update cportal_prod.ws_postmeta set meta_value='No' where post_id=v_postid and meta_key='hide_product';
else

	insert into cportal_prod.ws_postmeta select max(meta_id)+1,v_postid,'_hide_product','field_62aad9efa1f05'  from cportal_prod.ws_postmeta;  
    insert into cportal_prod.ws_postmeta select max(meta_id)+1,v_postid,'hide_product','No' from cportal_prod.ws_postmeta;
 end if;
...
begin
	...
	select max(id)+1 into v_postid from cportal_prod.ws_posts; 
    
	insert into cportal_prod.ws_posts
...
begin
	...
	select count(*),trid into v_count_trans,v_trid_main_post  from cportal_prod.ws_icl_translations 
    ...
    begin
		select max(trid) + 1 into v_next_trid from cportal_prod.ws_icl_translations; 
...
begin
    ...
    begin
		...
		if(select count(*)  from cportal_prod.ws_icl_translations where element_type = 'post_product'  collate utf8mb4_unicode_ci and element_id = v_postid and trid=v_trid_main_post and language_code=p_lang collate utf8mb4_unicode_ci)=0 then
			begin
				insert into cportal_prod.ws_icl_translations (element_type, element_id,trid, language_code,source_language_code)
...
select id into v_postid from cportal_prod.ws_posts p where post_type='product_variation' and post_excerpt=ifnull(p_attribute,'') collate utf8mb4_unicode_ci  and post_parent=p_parent_id limit 1;
select post_title into v_title from cportal_prod.ws_posts where id=p_parent_id;
...
begin
	...
	if v_count_trans =0 then
    begin
		select max(trid) + 1 into v_next_trid from cportal_prod.ws_icl_translations where language_code = p_lang  collate utf8mb4_unicode_ci;
...
begin
    ...
    if
		(select count(*)  from cportal_prod.ws_icl_translations 
...
left join cportal_prod.ws_postmeta wp_pm on wp_pm.meta_key=pm.wp_name  collate utf8mb4_unicode_ci  and wp_pm.post_id=v_postid
...
select ter.slug,ter.name into v_term_slug,v_term_name from cportal_prod.ws_term_taxonomy tx inner join cportal_prod.ws_terms ter on ter.term_id=tx.term_id where tx.term_taxonomy_id=p_term_taxonomy_id;
...
select id into v_postid from cportal_prod.ws_posts p where post_type='product_variation' and post_excerpt=concat(ifnull(p_attribute,''),': ',v_term_name) collate utf8mb4_unicode_ci  and post_parent=p_parent_id limit 1;
select post_title into v_title from cportal_prod.ws_posts where id=p_parent_id;
...
insert into cportal_prod.ws_postmeta select max(meta_id)+1,v_postid,concat('attribute_pa_',lower(p_attribute)),v_term_slug  from cportal_prod.ws_postmeta  ;    
...
from cportal_prod.ws_term_taxonomy tx 
inner join cportal_prod.ws_terms ter on ter.term_id=tx.term_id  collate utf8mb4_unicode_ci
...
INSERT INTO cportal_prod.ws_terms
(`term_id`,`name`,`slug`,`term_group`)
values(v_term_id,p_term_name,p_term_name,0); 

select max(term_taxonomy_id)+1 into v_term_taxonomy_id from cportal_prod.ws_term_taxonomy;

INSERT INTO cportal_prod.ws_term_taxonomy
...
from cportal_prod.ws_term_relationships rel inner join 
cportal_prod.ws_term_taxonomy tx on tx.term_taxonomy_id=rel.term_taxonomy_id collate utf8mb4_unicode_ci
...
begin
	INSERT INTO cportal_prod.ws_term_relationships
...
if v_log=1 then insert into temp_debug(value) values(concat('start sp_AddComplianceToProduct p_postid:',p_postid,' p_prop_trans:',p_prop_trans)); end if;
            ...
            set v_meta_id=0;
            select id into v_meta_id from cportal_prod.ws_posts where post_status='publish' and post_excerpt='product_complance' and post_title='Compliance' and post_content like '%v_current_compliance%';
            if (v_meta_id>0) then
				update cportal_prod.ws_posts set post_content=
				concat(
...
declare v_post_excerpt,v_prop_trans,v_prop_trans_value,v_propertytranslation,v_contactmedialist,v_contactstr text;
declare v_product_type,v_working_contact_media varchar(100);
...
from pim_wp_mapping m
right join pim_wp_mapping m1 on m.wp_name=concat('_',m1.wp_name) and m.wp_area='post property'  and m.description='field mapping'
...
begin
 	...
 	delete from cportal_prod.ws_postmeta where post_id=v_postid and meta_key not in ('_price','_regular_price','sellable','_sellable');
...
delete r from cportal_prod.ws_term_relationships r 
inner join cportal_prod.ws_term_taxonomy tx on tx.term_taxonomy_id=r.term_taxonomy_id
inner join cportal_prod.ws_terms t on t.term_id=tx.term_id where taxonomy='product_cat' and r.object_id=v_postid;
delete r from cportal_prod.ws_term_relationships r 
inner join cportal_prod.ws_term_taxonomy tx on tx.term_taxonomy_id=r.term_taxonomy_id
inner join cportal_prod.ws_terms t on t.term_id=tx.term_id where taxonomy='product_group' and r.object_id=v_postid;
delete r from cportal_prod.ws_term_relationships r 
inner join cportal_prod.ws_term_taxonomy tx on tx.term_taxonomy_id=r.term_taxonomy_id
inner join cportal_prod.ws_terms t on t.term_id=tx.term_id where taxonomy='product_line' and r.object_id=v_postid;
delete r from cportal_prod.ws_term_relationships r 
inner join cportal_prod.ws_term_taxonomy tx on tx.term_taxonomy_id=r.term_taxonomy_id
inner join cportal_prod.ws_terms t on t.term_id=tx.term_id where taxonomy='product_custom_type' and r.object_id=v_postid;
...
inner join cportal_prod.ws_terms ter on ter.term_id=tx.term_id
inner join cportal_prod.ws_icl_translations icl on  icl.element_id=ter.term_id
left join cportal_prod.ws_icl_translations t on t.element_id=tx.term_id
...
if JSON_UNQUOTE(json_extract(json_extract(json_extract(p_post_json,'$[0]'),'$.dash_number'),'$.name')) is not null then
	select fn_SerializeDashNumber(JSON_UNQUOTE(json_extract(json_extract(json_extract(p_post_json,'$[0]'),'$.dash_number'),'$.name')),v_log) into v_meta_id;
end if;
...
select meta_value into v_working_contact_media from cportal_prod.ws_postmeta where post_id=v_postid and meta_key='product_contact_media';
if v_contactmedialist!='' then
begin
set v_contactmedialist=concat(v_contactmedialist,',',v_working_contact_media);
end; 
else
begin
set v_contactmedialist=v_working_contact_media;
end;
end if;
 ...
 select fn_AddPostMetaValue_v10(v_postid,'_product_contact_media','field_655f1068564e6',v_log) into v_meta_id;
set v_count=0;
set v_contactstr='';

while SUBSTRING_INDEX(v_contactmedialist,',',1)<> '' do
begin
set v_working_contact_media=SUBSTRING_INDEX(v_contactmedialist,',',1);
set v_contactmedialist=replace(v_contactmedialist,v_working_contact_media,''); 
if v_contactmedialist like ',%' then set v_contactmedialist=substring(v_contactmedialist,2); end if; 
set v_contactstr=concat(v_contactstr,'i:',v_count,';s:',length(v_working_contact_media),':"',v_working_contact_media,'";');


set v_count=v_count+1;
end;
end while;
set v_contactstr=concat('a:',v_count,':{',v_contactstr,'}');
select fn_AddPostMetaValue_v10(v_postid,'product_contact_media',v_contactstr,v_log) into v_meta_id;
...
insert into cportal_prod.ws_postmeta (post_id,meta_key,meta_value)
select v_postid,wp_name,value from pim_wp_mapping m
left join cportal_prod.ws_postmeta pm on pm.meta_key=m.wp_name and pm.post_id=v_postid
...
set v_price=ifnull(v_price,1000);
	...
	insert into cportal_prod.ws_postmeta(post_id,meta_key,meta_value) values(v_postid,'_regular_price',v_price);
    
    set v_sku=fn_GetProperty_v10(p_post_json,'Product Code',p_lang,v_log);
    insert into cportal_prod.ws_postmeta(post_id,meta_key,meta_value) values(v_postid,'_sku',v_sku);
...
begin
     ...
     INSERT INTO cportal_prod.ws_woocommerce_attribute_taxonomies (`attribute_label`, `attribute_name`, `attribute_type`, `attribute_orderby`, `attribute_public`) VALUES (p_attr,lower(replace( p_attr,' ','-')), 'select', 'menu_order', 0);
...
inner join cportal_prod.ws_icl_translations icl on icl.element_id=tbl.attr_id
inner join cportal_prod.ws_terms ter on  icl.element_id=ter.term_id
inner join cportal_prod.ws_term_taxonomy tx on ter.term_id=tx.term_id
...
select id into v_postid from cportal_prod.ws_posts p 
inner join cportal_prod.ws_icl_translations icl on icl.element_id=p.id
...
select post_title into v_title from cportal_prod.ws_posts where id=p_parent;
...
update cportal_prod.ws_postmeta h inner join 
v_tbl_attributes tbl on h.meta_key=concat('attribute_',tbl.attr_type) collate utf8mb4_unicode_ci
inner join cportal_prod.ws_icl_translations icl on icl.element_id=tbl.attr_id
inner join cportal_prod.ws_terms ter on  icl.element_id=ter.term_id
inner join cportal_prod.ws_term_taxonomy tx on ter.term_id=tx.term_id  and h.meta_key=concat('attribute_',tx.taxonomy)
...
left join cportal_prod.ws_postmeta pm on pm.post_id=v_postid and meta_key='_sku'
...
declare v_metaid bigint;

declare v_material,v_scalefrom,v_scaleto,v_price100,v_baseprice,v_post_id,v_customer,v_isgeneric,v_working_customer,v_workmat varchar(200);

DECLARE material_cursor CURSOR FOR 
select material,ScaleFrom,ScaleTo,Price100,BasePrice,post_id,case when Customer='1024495' then '' else Customer end as Customer,user_id,IsGeneric  from (
select distinct s.material,s.ScaleFrom,s.ScaleTo,s.Price100,s.BasePrice,pmo.post_id,s.Customer,um.user_id,s.IsGeneric 
from cportal_prod.ws_postmeta pmo inner join stg_prices s on s.Material=pmo.meta_value
 inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=s.customer
 where pmo.meta_key='_sku' 
 and s.customer<>''
 union
 select distinct s.material,s.ScaleFrom,s.ScaleTo,s.Price100,s.BasePrice,pmo.post_id,s.Customer,0,'true'
from cportal_prod.ws_postmeta pmo inner join stg_prices s on s.Material=pmo.meta_value
 where pmo.meta_key='_sku' 
 and s.customer='1024495'
 union
  select distinct s.material,s.ScaleFrom,s.ScaleTo,s.Price100,s.BasePrice,pmo.post_id,s.Customer,um.user_id,'false' as isgeneric
from 
cportal_prod.ws_postmeta pmo 
inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer'
inner join import_gi_prod.stg_prices s on s.Material=pmo.meta_value 
left join cportal_prod.ws_wusp_user_pricing_mapping pm on pm.product_id=pmo.post_id and um.user_id=pm.user_id
where s.isgeneric='true' and s.customer='' and pm.id is null  and um.meta_value<>'' and pmo.meta_key='_sku' and material not like '44%'
 ) x  order by material,x.Customer,user_id;
...
if v_workmat<>v_material then
	begin
		set v_workmat=v_material;
        insert into temp_debug(value) values(concat('sp_ImportPrices-> starting price for material:',v_material));
    end;
end if;
...
begin
    ...
    delete m from  cportal_prod.ws_wusp_user_pricing_mapping m inner join cportal_prod.ws_posts p on p.id=m.product_id inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=v_customer and m.user_id=um.user_id 
and v_workmat=p.post_title;
    
    if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices-> deleting custom prices for:',v_material,' v_customer:',v_customer)); end if;
...
if v_finished =1 then 
 ...
 begin
	...
	insert into cportal_prod.ws_postmeta (post_id,meta_key,meta_value)
	select v_post_id,'_price',v_price100/100 from dual where not exists (select meta_id from cportal_prod.ws_postmeta where meta_key='_price' and post_id=v_post_id)
	union
	select v_post_id,'_regular_price',v_price100/100 from dual where not exists (select meta_id from cportal_prod.ws_postmeta where meta_key='_regular_price' and post_id=v_post_id);
    
    set v_metaid=0;
    select meta_id into v_metaid from cportal_prod.ws_postmeta where post_id=v_post_id and meta_key='_sellable';
    if v_metaid=0 then
		insert into cportal_prod.ws_postmeta (post_id,meta_key,meta_value) values(v_post_id,'_sellable','field_66c6dca3729e8');
	end if;
    set v_metaid=0;
    select meta_id into v_metaid from cportal_prod.ws_postmeta where post_id=v_post_id and meta_key='sellable';
    if v_metaid=0 then
		
        insert into cportal_prod.ws_postmeta (post_id,meta_key,meta_value) values(v_post_id,'sellable','1');
	else
		update cportal_prod.ws_postmeta set meta_value='1' where meta_id=v_metaid;
	end if;
...
INSERT INTO cportal_prod.ws_wusp_user_pricing_mapping
...
DELIMITER $$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `sp_ImportPricesFromSAP_v00`(v_log bit)
BEGIN
...
from cportal_prod.ws_postmeta pmo inner join stg_prices s on s.Material=pmo.meta_value
 inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=s.customer
 where pmo.meta_key='_sku' 
 and s.customer<>''  and s.customer=1016656
...
if (v_working_customer<>v_customer and v_customer<>'') then
begin
    delete m from  cportal_prod.ws_wusp_user_pricing_mapping m inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=v_customer and m.user_id=um.user_id;
    if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices-> deleting custom prices for:',v_material,' v_customer:',v_customer)); end if;
    set v_working_customer=v_customer;
end;
end if;
...
begin 
	...
	delete p,icl,pm from cportal_prod.ws_posts p
	inner join cportal_prod.ws_icl_translations icl on icl.element_id=p.id
	inner join cportal_prod.ws_postmeta pm on pm.post_id=p.id
...
if v_work_lang='en' then 
    ...
    begin
        ...
        delete from cportal_prod.ws_postmeta where post_id=v_postid and meta_key='_product_image_gallery';
	...
	while v_counter_images<v_total_images do
        ...
        select meta_key into v_file_metakey from cportal_prod.ws_postmeta  where meta_key like 'pdp_document_%' and meta_value=v_file_dsc and post_id=v_postid;
        set v_file_metakey = left(v_file_metakey ,length(v_file_metakey )-5);
        
        if  v_file_metakey is null then 
			select count(1) into v_count_file_dsc from cportal_prod.ws_postmeta where post_id=v_postid and meta_key like 'pdp_document%_title';
        ...
        select count(1) into v_count_file_dsc from cportal_prod.ws_postmeta  where meta_key like concat(v_file_metakey,'%','_file') and post_id=v_postid; 
...
BEGIN
    ...
    DECLARE v_finished INTEGER DEFAULT 0;
    declare v_work_meta_id,v_work_invoice bigint;
    DECLARE invoice_cursor CURSOR FOR 
    SELECT order_number FROM stg_invoices where importid=(select max(importid) from stg_invoices);
	...
	loop_invoice: LOOP
		...
		select pm.meta_id into v_work_meta_id from cportal_prod.ws_postmeta pm inner join stg_invoices i on i.order_number=pm.post_id and pm.meta_key='sap_invoice';
		if v_work_meta_id is not null then
		begin
			update cportal_prod.ws_postmeta pm 
			inner join stg_invoices i on i.order_number=pm.post_id
			set meta_value=i.pdt_path
...
DELIMITER $$
CREATE DEFINER=`admin`@`localhost` PROCEDURE `sp_insertStocksIntoPortal`()
BEGIN


	update import_gi_prod.stg_stocks stgs 
	inner join cportal_prod.ws_postmeta pm on pm.meta_value=stgs.material
	inner join cportal_prod.ws_postmeta pms on pm.post_id=pms.post_id and pms.meta_key='_stock'
	set pms.meta_value=stgs.StockQuantity
	where pm.meta_key='_sku' and   stgs.importid=(select max(importid) from import_gi_prod.stg_stocks);

	insert into cportal_prod.ws_postmeta (post_id,meta_key,meta_value)
	select pm.post_id,'stock',stgs.StockQuantity
	from
	 import_gi_prod.stg_stocks stgs 
	inner join cportal_prod.ws_postmeta pm on pm.meta_value=stgs.material
	left join cportal_prod.ws_postmeta pms on pm.post_id=pms.post_id and pms.meta_key='_stock'
	where  pm.meta_key='_sku' and  stgs.importid=(select max(importid) from import_gi_prod.stg_stocks) and pms.meta_id is null;
    
    
update import_gi_prod.stg_stocks stgs 
right join (cportal_prod.ws_postmeta pm   
inner join cportal_prod.ws_postmeta pms on pm.post_id=pms.post_id and pms.meta_key='_stock'  ) on  pm.meta_value=stgs.material and stgs.importid=(select max(importid) from import_gi_prod.stg_stocks)
...
while v_counter<v_total do
	...
	if v_log=1 then 
    insert into temp_debug(value) values(concat('sp_migrate_data_from_pim_v11-> iterating posts -> post:',v_post_json ));
    end if;
    ...
    if v_log=1 then 
    insert into temp_debug(value) values(concat('sp_migrate_data_from_pim_v11 -> imported '));
    end if;
end while;
...
DELIMITER $$
CREATE DEFINER=`jmartins`@`localhost` PROCEDURE `sp_updatePricesForUser`(p_user_id bigint,p_sap_number varchar(20),v_log bit)
BEGIN
...
from cportal_prod.ws_postmeta pmo inner join stg_prices s on s.Material=pmo.meta_value
 inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=s.customer
 where pmo.meta_key='_sku' 
 and s.customer=p_sap_number and um.user_id=p_user_id and material not like '44%' 
 union
  select distinct s.material,s.ScaleFrom,s.ScaleTo,s.Price100,s.BasePrice,pmo.post_id,s.Customer,um.user_id,'false' as isgeneric
from 
cportal_prod.ws_postmeta pmo 
...
inner join import_gi_prod.stg_prices s on s.Material=pmo.meta_value 
left join cportal_prod.ws_wusp_user_pricing_mapping pm on pm.product_id=pmo.post_id and um.user_id=pm.user_id
where s.isgeneric='true' and s.customer='' and pm.id is null  and um.meta_value<>'' and pmo.meta_key='_sku' and material not like '44%'  and um.user_id=p_user_id
 ) x  order by material,x.Customer,user_id;
...
DECLARE exit handler for SQLEXCEPTION
 BEGIN
  GET DIAGNOSTICS CONDITION 1 @sqlstate = RETURNED_SQLSTATE, 
   @errno = MYSQL_ERRNO, @text = MESSAGE_TEXT;
  SET @full_error = CONCAT("ERROR procedure sp_ImportPrices for user : ", @errno, " (", @sqlstate, "): ", @text);
  insert into temp_debug(value) values(@full_error);
 END;
...
delete from cportal_prod.ws_wusp_user_pricing_mapping  where user_id=p_user_id;
...
if v_workmat<>v_material then
	begin
		set v_workmat=v_material;
        insert into temp_debug(value) values(concat('sp_ImportPrices for user-> starting price for material:',ifnull(v_material,'xxx')));
    end;
end if;

if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices for user-> v_work_mat:',v_material,' v_finished:',v_finished)); end if;

if (v_working_customer<>v_customer and v_customer<>'') then
begin
    
    delete m from  cportal_prod.ws_wusp_user_pricing_mapping m inner join cportal_prod.ws_posts p on p.id=m.product_id inner join cportal_prod.ws_usermeta um on um.meta_key='sap_customer' and um.meta_value=v_customer and m.user_id=um.user_id 
and v_workmat=p.post_title and um.user_id=v_user_id;
    
    if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices for user-> deleting custom prices for:',v_material,' v_customer:',v_customer)); end if;
    set v_working_customer=v_customer;
end;
end if;
...
if v_finished =1 then 
 ...
 if v_isgeneric='true' then
 begin
	if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices for user-> updating standard prices:',v_material,' v_customer:',v_customer,' v_user_id:',v_user_id)); end if;
    end;
else
begin
...
(product_id,user_id,price,min_qty,flat_or_discount_price)
VALUES(v_post_id,v_user_id,v_price100/100,v_scalefrom,1);
if v_log=1 then insert into temp_debug(value) values(concat('sp_ImportPrices for user for user-> inserted prices:',v_material,' v_customer:',v_customer,' v_user_id:',v_user_id,' v_scalefrom:',v_scalefrom)); end if;
end;
end if; 
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `zz_sp_CreateMainPost_v10`(p_post_json longtext,p_parent_id bigint,p_lang varchar(2),v_log bit,out postid bigint)
BEGIN
...
begin
 	...
 	delete from cportal_prod.ws_postmeta where post_id=v_postid;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `zz_sp_CreateMainPost_v11`(p_post_json longtext,p_parent_id bigint,p_lang varchar(2),p_integration_number bigint,v_log bit,out postid bigint)
BEGIN

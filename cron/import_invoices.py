...
importdatabase='import_gi'
importdbuser='jmartins'
...
#SourcePath = '/datadrive/pim-gi/storage/app/public/' #C:/Users/jmart/Downloads/imagens/imagens/'
...
Targetpath= '/datadrive/cabling-webshop/wp-content/uploads/woocommerce_uploads/'
...
def getNewInvoices():
    try:
        ...
        importquery = 'delete from import_gi.stg_invoices;'
        importcursor.execute(importquery)
        
        myresult = stgcursor.fetchall()

        for row in myresult:
            importquery = "INSERT INTO import_gi.stg_invoices(invoice_number, order_number,customer_number,pdf_path,importid) VALUES('"+str(row[1])+"','"+str(row[2])+"','"+str(row[3])+"','"+str(row[4])+"',"+str(maxid)+");"
            importcursor.execute(importquery)
        ...
        importquery = "call sp_SyncInvoices(1);"
        importcursor.execute(importquery)
        importmydb.commit()
        importcursor.close()

...
<div class="cart_totals <?php echo ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : ''; ?>">
	...
	<table cellspacing="0" class="shop_table shop_table_responsive">
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) {
			...
			// Not show tax at cart page
			if( false ){
				if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) {
					foreach ( WC()->cart->get_tax_totals() as $code => $tax ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
						?>
						<tr class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
							<th colspan="2"><?php echo esc_html( $tax->label ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></th>
					...
					?>
					<tr class="tax-total">
						<th colspan="2"><?php echo esc_html( 'Sales Tax' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></th>
						<td data-title="Sales Tax"><?php wc_cart_totals_taxes_total_html(); ?></td>
					</tr>
					<?php
				}
			}

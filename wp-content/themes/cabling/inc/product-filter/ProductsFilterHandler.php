...
class ProductsFilterHandler {
	...
	private array $surfaceLineAvailable = [];
	...
	private function processRequest(): void {
		// GE-265 fix filter
		if ( !empty($this->data['product_line'])  && empty($this->categoryType)) {
			...
			if ($productTypeId) {
				ob_start();
				echo '<div class="row">';
				foreach ($productTypeId as $value) {
					$this->productTypeId = $value;

					$this->results = '';
					$this->total   = 0;

					$this->processProductTypeCustom();

					$allResults .= $this->results;
					$allTotals  += $this->total;
				}
        ...
        wp_send_json_success([
			...
			'surfaceLineAvailable' => $this->surfaceLineAvailable,
			//'$this->surfaceIds' => $this->surfaceIds ?? null,
		]);
	}

	// GE-265 fix filter
	private function processProductTypeCustom(): void {
		...
		if ($productType) {
			if ( empty($this->categoryType) ) {
				...
				foreach ($skuTypes as $type) {
					...
					if ( $skuList) {
						$term_id   = $productType->term_id; 
						$sku_type  = $type; 
						$string = $_POST['data'];
						parse_str($string, $parsed);
						$referer = $parsed['_wp_http_referer'] ?? '';
						$data_history = null;
						if ($referer) {
							$parts = parse_url($referer);
							if (!empty($parts['query'])) {
								parse_str($parts['query'], $referer_query);
								if (!empty($referer_query['data-history'])) {
									$data_history = $referer_query['data-history'];
								}
							}
						}
						$history = !empty($data_history) ? json_decode(base64_decode($data_history), true) : [];
						$sellable = isset($history['attributes']['sellable'][0]) ? (int) $history['attributes']['sellable'][0] : 1;
						$has_items = $this->check_product_by_custom_type_and_sku( $term_id, $sku_type, $sellable );
						if ( $has_items ) {
	...
	// Fix filter
	private function check_product_by_custom_type_and_sku( $term_id, $sku_type, $sellable ) {
		$args = array(
			'post_type'      => 'product',
			'fields'         => 'ids', // return only IDs for efficiency
			...
			'meta_query'     => array(
				'relation' => 'AND', // make sure both conditions apply
				array(
					'key'     => 'sku_type',
					'value'   => $sku_type,
					'compare' => '=',
		...
		return $query->posts; // array of product IDs
	}
...
//JM 20250831 fix for when no sellable skus are available for the product line picked
	...
	private function getSurfaceLineAvailable(): ?array {
		$productGroupIds = $this->getProductGroupIds();
		$this->product_ids = ProductsFilterHelper::searchProductByMeta($this->data['attributes'], $this->group, $this->surfaceIds, $this->filterTaxonomy);
        ...
        if (!empty($productGroupIncludes)) {
            $available = [];
            $lines = ProductsFilterHelper::getProductLineCategory('product_line', 'group_category', $productGroupIds, false, $productGroupIncludes);
            if (!empty($lines)) {
                foreach ($lines as $line) {
                    $available[] = $line->term_id;
                }
            }
            return $available;
        }
	...
	private function processProductMetaData(): void {
		...
		if (!$isSurfaceEquipment) {
            ...
            if (!empty($this->data['attributes']['sellable'])) {
                $this->surfaceLineAvailable = $this->getSurfaceLineAvailable();
            }
        }
	}

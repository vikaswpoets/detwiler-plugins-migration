...
function cabling_additional_information()
{
    ...
    if ($is_surface_equipment){
        $key_features = get_field('surface_key_features', $product->get_id());
...
function cabling_surface_information() {
    ...
    if (is_product_in_cart($productId ?? 0)){
        cabling_add_quote_section();
    }
    if (gi_product_has_surface_equipment($productId)){
        $productLines = get_the_terms( $productId, 'product_line' );
        if (!empty($productLines[0])){
	        $productTypes= get_the_terms( $productId, 'product_custom_type' );
            $replacementPartHeading = sprintf(__('for %s Models', 'cabling'), $productTypes[0]->name ?? '');
		    $typeOfProduct = get_field('sku_type', $productId);

            if (empty( $typeOfProduct)){
                return;
            }

	        $skuProductType = ProductsFilterHelper::getProductSkuType();
	        $skuReplacementTypes = ProductsFilterHelper::getReplacementSkuType();

            $mainProducts = array();
	        if ( $typeOfProduct == $skuProductType ) {
		        //get the list of popular sku
		        $mainProducts[ $skuProductType ] = get_component_products( $productTypes[0]->term_id ?? 0, $skuProductType, $productId );

		        //get the list of components
		        $componentProducts = array();
		        foreach ( $skuReplacementTypes as $skuType ) {
			        $list = get_component_products( $productTypes[0]->term_id ?? 0, $skuType, $productId );
			        if ( ! empty( $list ) ) {
				        $componentProducts[ $skuType ] = $list;
			        }
		        }
	        } else {
		        $list = get_component_products( $productTypes[0]->term_id ?? 0, $typeOfProduct, $productId );
		        if ( ! empty( $list ) ) {
			        $mainProducts[ $typeOfProduct ] = $list;
		        }
	        }
	        ...
	        wc_get_template('single-product/product-surface-details.php', array(
		        ...
		        'componentProducts' => $componentProducts ?? null,
 ...
 */
function get_component_products( int $productTypeId, string $SKU_TYPE_COMPONENT, int $productId = 0 ) {
	$args = [
		'post_type'      => 'product',
		'posts_per_page' => -1,
		'fields'         => 'ids',
...
function render_doublee_sku_table( $attributes, $products ) {
    if ( empty( $products ) ) {
        return;
    }
	$col_number     = 0;
	foreach ( $products as $productId ):
		...
		foreach ( $attributes as $key => $attribute ) {
			$row_values[] = get_product_field( $key, $product->get_id() );
		}
		// Check if all are empty
		$hide_row = ! empty( $row_values ) && count( array_filter( $row_values, fn( $v ) => $v !== '*' ) ) === 0;
		if ( $hide_row ) {
			continue;
		}
		?>
        <tr class="product-row">
			<?php foreach ( $attributes as $key => $attribute ): $col_number ++; ?>
				<?php if ($key === '_sku'): ?>
                    <td class="has-text-align-center" data-align="center"><?php echo get_the_title($product->get_id()) ?></td>
                <?php else: ?>
					<?php $value = get_product_field( $key, $product->get_id() ); ?>
                    <td class="has-text-align-center" data-align="center"><?php echo $value ?? '*' ?></td>
				<?php endif ?>
			<?php endforeach ?>
            <td
            ...
            >
                <div class="d-flex justify-content-center">
					<?php if ( empty( $product->get_price() ) ): ?>
                        <div data-action="<?php echo esc_attr( $product->get_id() ); ?>"
					...
					<?php endif ?>
                    <a href="#"
                       class="add-to-cart-button add-to-wishlist ms-2 mb-0"
                       data-product="<?php echo esc_attr( $product->get_id() ); ?>">
                        <i class="fa-light fa-heart me-2"></i>
                        <span style="font-size: 10px"><?php echo __( 'Add to wishlist', 'cabling' ); ?></span>
                    </a>
                </div>
            </td>
        </tr>
...
function cabling_woocommerce_show_surface_equipment_meta( $product_id ) {
    $meta_data = get_surface_equipment_meta( $product_id );

    wc_get_template('single-product/product-doublee-meta.php', array(
        'data' => $meta_data,
    ));
}
function get_surface_equipment_meta( $product_id ) {
	$product_attributes = array(
		'_sku'                        => __( 'Name', 'cabling' ),
		'surface_rubber_type'         => __( 'Rubber Type', 'cabling' ),
		'surface_thread_up'           => __( 'Thread Up', 'cabling' ),
    ...
    foreach ( $product_attributes as $key => $attribute){
        if ($key === '_sku'){
            ...
            $value = get_product_field($key, $product_id);
        }

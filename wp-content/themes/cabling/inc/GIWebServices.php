...
class GIWebServices
{
    ...
    {
        $cachedKey = 'gi_webservices_access_token';
        $cachedToken = get_transient($cachedKey);
        ...
        if (is_wp_error($response)) {
            ...
            if (isset($data['access_token'])) {
                ...
                // Cache the token
                set_transient($cachedKey, $tokenData, $expiration);
    ...
    public function makeApiRequest($apiEndpoint, array $params, $payload = []): array
    {
        try {
			...
			//wp_mail('jose.martins@infolabix.com', 'makeApiRequest request '.$endpoint, json_encode($bodyParams));
            $response = $this->curl($endpoint, $bodyParams, $headers);
			//wp_mail('jose.martins@infolabix.com', 'makeApiRequest', json_encode($response));

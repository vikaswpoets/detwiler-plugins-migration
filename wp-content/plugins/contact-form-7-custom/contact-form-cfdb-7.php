<?php
/*
Plugin name: Contact Form Database
...
Text Domain: contact-form-7-custom
...
add_action( 'admin_init', 'send_contact_reply');
function send_contact_reply() {
    if (
        isset($_POST['_contact_nonce']) &&
        wp_verify_nonce($_POST['_contact_nonce'], 'reply_form_contact')
    ) {
        $mailer = WC()->mailer();

        $to = sanitize_email($_POST['to']);
        $subject = sanitize_text_field($_POST['subject']);
        $content = wp_kses_post($_POST['message']);
        ...
        $mailer->send($to, $subject, $mail_content, $headers, $attachments);
        ...
        if (!empty($results[0])){
            ...
            $form_data['cfdb7_status'] = 'replied';
            $form_data['reply_content'] = array(

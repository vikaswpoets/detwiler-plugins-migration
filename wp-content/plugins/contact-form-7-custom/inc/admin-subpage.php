...
class CFDB7_List_Table extends WP_List_Table
{
    ...
    /**
     * Define reply_content
     * @param  $item
     */
    public function column_reply_content($item){
        if (empty($item['reply_content'])) return '-';

        return printf('%s <br> %s', $item['reply_content']['date'], $item['reply_content']['title']);
    }
    /**
    ...
    {
       return array(
          'form-date' => array('form-date', true)
       );
    }
    /**
     * Define bulk action
    ...
    public function get_bulk_actions() {
        ...
        return array(
            ...
            'replied' => __( 'Replied', 'contact-form-cfdb7' ),
    ...
    private function table_data()
    {
        ...
        foreach ( $results as $result ) {
            ...
            foreach ( $this->column_titles as $col_title) {
                $form_value[ $col_title ] = $form_value[$col_title] ?? '';
            }
            ...
            $form_values['reply_content'] = $form_value['reply_content'] ?? [];
            $data[] = $form_values;
        }
    ...
    public function process_bulk_action(){
        ...
        if( 'delete' === $action ) {
            ...
            foreach ($form_ids as $form_id):
                ...
                $result_value  = $results[0]->form_value;
                $result_values = unserialize( $result_value );
                $result_values['cfdb7_status'] = 'replied';
                $form_data = serialize( $result_values );
                $cfdb->query(
                    "UPDATE $table_name SET form_value = '$form_data' WHERE form_id = '$form_id'"
                );

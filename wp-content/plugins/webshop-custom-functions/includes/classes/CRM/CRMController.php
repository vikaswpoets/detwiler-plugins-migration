...
class CRMController
{
	...
	{
		$cache_key = 'crm_csrf_token_' . md5($this->username . $this->password . $this->baseURL);
        //the idea is nice, but we don't know how long it expires and if we get an error on token validation, it is not requesting the token again
        // must be reviewed
		...
		try {
			...
			set_transient($cache_key, $lst, 29 * HOUR_IN_SECONDS);
    ...
    {
        // use special user
        return $this->createPostHeaderNoToken();
        $credentials = $this->encodeCredentials();
        $headers = [
            'content-type' => 'application/json',
            'authorization' => 'Basic ' . $credentials,
    ...
    private function createPostHeaderNoToken()
    {
        $this->username="_GIWEB";
        $this->password="D@twy!er123456789";
        $credentials = $this->encodeCredentials();
        $headers = [
            'content-type' => 'application/json',
            'authorization' => 'Basic ' . $credentials,
    ...
    private function makePostRequest($url, $headers, $body)
    {
        ...
        if (is_wp_error($response)) {
            ...
            send_notify_error('[CRM makePostRequest ERROR]', $response->get_error_message(),'crm');
            return [];
        } else {
    ...
    private function makeGetRequest($url)
    {
        ...
        if (is_wp_error($response)) {
            ...
            send_notify_error('[CRM makeGetRequest ERROR]', $response->get_error_message(),'crm');
            return [];
        } else {
            // Get the response body
    ...
    private function makeGetRequestWithAllResults($url)
    {
        $response = wp_remote_get($url, array(
            'headers' => $this->createGetHeader(),
            'timeout' => 60
    ...
    private function getAllSalesOrg($objectid)
    {
        //$client = new Client(); //GuzzleHttp\Client
        $url=$this->baseURL."CorporateAccountCollection('".$objectid."')/CorporateAccountSalesData";
        $url=$url.'?$format=json';
        $response = $this->makeGetRequestWithAllResults($url);
        return $response;
    }

    public function getMultipleSalesOrganization($accountid)
    {
        $res=$this->getAccount($accountid);
        $lst=[];
        if (isset($res->ObjectID)){
            $objectid=$res->ObjectID;
            $salesres=$this->getAllSalesOrg($objectid);
            foreach($salesres as $item)
            {
                if(isset($item->SalesOrganisationID) && isset(CRMConstant::NAMEDSALESORGS[$item->SalesOrganisationID]))
                $lst[]=CRMConstant::NAMEDSALESORGS[$item->SalesOrganisationID];
            }
        }
        return $lst;
    }
     ...
     * @author José Martins
     * @date 20250804
     * @description Retrieves Customer Group Code and Customer Price List Code to use in Price Synchronization
     * This is used at login stage
     * @return Returns array with list of customer groups and list of price codes
     **/
    public function getCustomerPriceDetails($accountid)
    {
        //$accountid="1039446";
        $res=$this->getAccount($accountid);
        $lstcustomergroup=[];
        $lstpricelist=[];
        $lstpricelistperSalesOrg=[];
        if (isset($res->ObjectID)){
            $objectid=$res->ObjectID;
            $salesres=$this->getAllSalesOrg($objectid);
            foreach($salesres as $item)
            {
                if(isset($item->CustomerGroupCode) )
                    $lstcustomergroup[]=$item->CustomerGroupCode;
                if(isset($item->PriceListCode) )
                {
                    $lstpricelist[]=$item->PriceListCode;
                    if(isset($item->SalesOrganisationID) )
                    {
                        //$lstpricelistperSalesOrg[]=$item->SalesOrganisationID;
                        if (strlen($item->SalesOrganisationID)>4){
                            $lstpricelistperSalesOrg[substr($item->SalesOrganisationID,-4)]=$item->PriceListCode;
                        }else
                        {
                            $lstpricelistperSalesOrg[$item->SalesOrganisationID]=$item->PriceListCode;
                        }
                    }
        ...
        $lst['PriceListCode']=$lstpricelist;
        $lst['CustomerGroupCode']=$lstcustomergroup;
        $lst['lstpricelistperSalesOrg']=$lstpricelistperSalesOrg;
        return $lst;
    }
    ...
    private function getContactByEmail($email)
    {
        ...
        $url = $url . '?$filter=Email eq \'' . $email . '\' and StatusCode eq \'2\'';
        $url = $url . '&$format=json';
    ...
    {
        //returning without using token
        // activate when ready
        //return $this->createContactUsLeadNoToken($crmcontact, $comments, $productofinterest);
     ...
     * Fill the CRMcontact Object before requesting the lead
     */
    protected function createContactUsLeadNoToken($crmcontact, $comments, $productofinterest)
    {
        ...
        $url = $this->baseURL . "LeadCollection";
        $headers = $this->createPostHeaderNoToken();
